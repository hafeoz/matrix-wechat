...
func fnSync(ce *WrappedCommandEvent) {
	...
	if space {
		...
		if !ce.Bridge.Config.Bridge.SpaceForOfficialAccounts {
			count := 0
			chatsToAdd := ce.Bridge.DB.Portal.FindAllChatsNotInSpace(ce.Ctx, ce.User.UID)
			...
			privateChatsToAdd := ce.Bridge.DB.Portal.FindPrivateChatsNotInSpace(ce.Ctx, ce.User.UID)
			officialAccountsToAdd := ce.Bridge.DB.Portal.FindOfficialAccountsNotInOASpace(ce.Ctx, ce.User.UID)
			officialAccountsToRemove := ce.Bridge.DB.Portal.FindOfficialAccountsInDefaultSpace(ce.Ctx, ce.User.UID)
			privateAdded := 0
			officialAdded := 0
			officialRemoved := 0
			...
			for _, key := range officialAccountsToAdd {
				portal := ce.Bridge.GetPortalByUID(key)
				portal.addToOfficialAccountSpace(ce.Ctx, ce.User)
				officialAdded++
			}
			for _, key := range officialAccountsToRemove {
				portal := ce.Bridge.GetPortalByUID(key)
				portal.removeFromSpace(ce.Ctx, ce.User)
				officialRemoved++
			}
